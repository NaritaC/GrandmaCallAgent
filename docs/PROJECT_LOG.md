...
### 2026-07-20T20:43:19+08:00 - V0-A snapshot gate and V1 control boundary implemented

- Category: implementation and decision
- Summary: Reworked the active route into V0-A fixed-matrix incoming-call validation, V0.5 experimental outbound, and V1 restricted Agent control. V1 now excludes generic tap/type tools, keeps real-time auto-answer deterministic and local, limits cloud responsibilities to typed call requests/config/status/audit, and preserves the phone SafetyGate as the final veto. Added read-only ADB capture and structural analysis for all six voice/video and screen-state combinations, operator target confirmation, manual screenshot review, a machine-readable gate, and explicit live-action refusal switches.
- Artifacts: `AGENTS.md`, `README.md`, `GrandmaBridge/app/src/main/AndroidManifest.xml`, `GrandmaBridge/app/src/main/java/com/grandmacallagent/bridge/MainActivity.kt`, `GrandmaBridge/app/src/main/java/com/grandmacallagent/bridge/v0/V0AutomationRuntime.kt`, `scripts/v0_*.ps1`, `docs/ARCHITECTURE.md`, `docs/LOCAL_RUN.md`, `docs/PERMISSIONS.md`, `docs/REFERENCE_PROJECTS.md`, `docs/ROADMAP.md`, `docs/TEST_CHECKLIST.md`, `docs/V0_AUTOMATION_VALIDATION.md`, `docs/V0_PHONE_VALIDATION.md`, `docs/V0_TEST_RECORD_TEMPLATE.md`, `docs/V0_GKD_VALIDATION.md`
